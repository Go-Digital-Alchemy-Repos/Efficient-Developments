...
function ApplicationForm({ job }: { job: Job }) {
  ...
  return <section className="careers-application contact-form-card" aria-labelledby="application-title">
    <Heading as="h2" id="application-title" size="card">{job.sample ? 'Application preview' : 'Apply for this role'}</Heading>
    {job.sample && <p className="careers-notice">This is a reference posting, not an active vacancy. Applications are disabled until the client activates this role.</p>}
    {success ? <div role="status" className="careers-success"><h3>Application received</h3><p>{success}</p><Link to="/careers">Back to careers</Link></div> : <form className="contact-form" onSubmit={submit}>
      <fieldset disabled={pending || job.sample} className="careers-fieldset">
        ...
        <Button type="submit" disabled={pending || job.sample}>{pending ? 'Submitting…' : job.sample ? 'Reference posting — applications closed' : 'Submit application'} <span aria-hidden="true">↗</span></Button>
      </fieldset>
    </form>}
  </section>
}
...
export function CareerJobPage() {
  ...
  return <main id="main-content" className="careers-page"><Container>
    ...
    {loading ? <p role="status">Loading role…</p> : error ? <p role="alert">{error}</p> : !job ? <><Heading as="h1">Position unavailable</Heading><p>This position is no longer listed. Explore our other opportunities.</p></> : <>
      <header className="careers-heading careers-heading--role"><Eyebrow>{job.department}</Eyebrow><Heading as="h1" size="page">{job.title}</Heading><p>{job.location} · {job.employment}</p>{job.sample && <span className="careers-badge">Reference posting</span>}</header>
